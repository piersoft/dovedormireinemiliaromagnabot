<?php
/**
* Telegram Bot example for dovedormireinemiliaromagnabot
 ...
 function shell($telegram,$text,$chat_id,$user_id,$location,$reply_to_msg)
{
	date_default_timezone_set('Europe/Rome');
	$today = date("Y-m-d H:i:s");
	if (strpos($text,'@dovedormireinemiliaromagnabot') !== false) $text=str_replace("@dovedormireinemiliaromagnabot ","",$text);

	if ($text == "/start" || $text == "©️ Informazioni") {
		$img = curl_file_create('logoer.png','image/png');
		$contentp = array('chat_id' => $chat_id, 'photo' => $img);
		$telegram->sendPhoto($contentp);
		$reply = "Benvenuto. Per ricercare una struttura ricettiva della Regione Emilia Romagna, censita dal Dipartimento regionale del turismo dello sport e dello spettacolo, digita il nome del Comune oppure clicca sulla graffetta (📎) e poi 'posizione' . Puoi anche ricercare per parola chiave nel titolo anteponendo il carattere ?. Verrà interrogato il DataBase openData utilizzabile con licenza CC-BY presente su http://dati.emilia-romagna.it/dataset/strutture-ricettive-regionali-alberghiere-ed-extra-alberghiere--6005440e. In qualsiasi momento scrivendo /start ti ripeterò questo messaggio di benvenuto.\nQuesto bot è stato realizzato da @piersoft con il prezioso aiuto di @matteoenna e potete migliorare il codice sorgente con licenza MIT che trovate su https://github.com/piersoft/dovedormireinemiliaromagnabot.\nLa propria posizione viene ricercata grazie al geocoder di openStreetMap con Lic. odbl.Immagine derivata da wikipedia lic. CC-BY-SA\n";
		$content = array('chat_id' => $chat_id, 'text' => $reply,'disable_web_page_preview'=>true);
		$telegram->sendMessage($content);
		$log=$today. ",new chat started," .$chat_id. "\n";
		file_put_contents('/usr/www/piersoft/dovedormireinemiliaromagnabot/db/telegram.log', $log, FILE_APPEND | LOCK_EX);
...
if(strpos($text,'!') !== false) {
	...
	//			sleep (1);
		...
		//		sleep (1);
			...
			$homepage="";
$json_string=file_get_contents("/usr/www/piersoft/dovedormireinemiliaromagnabot/db/ricettive.json");
